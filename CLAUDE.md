...
- **user-service → Kafka via the outbox pattern.** user-service will publish `UserCreated` / `UserUpdated` / `UserDeleted` (and `AlertRule*`). Mechanism: a transactional `outbox` table in the `user_service` schema — the same `@Transactional` service method writes the domain change **and** the outbox row atomically; a separate relay (`@Scheduled` poller first, Debezium/CDC later) publishes rows to Kafka and marks them sent. **Never** call `kafkaTemplate.send()` inside the transactional method. One `user-events` topic, event type in a header.
- **Consumer idempotency.** device-service (and later others) subscribe to `user-events`; delivery is at-least-once, so every consumer must be idempotent (`DELETE WHERE user_id = ?` is). This — not any cross-service cascade — is how a deleted user's devices get cleaned up.
