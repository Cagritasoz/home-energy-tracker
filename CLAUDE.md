...
- **Layer 1 — unit tests** (no I/O, no Spring context, milliseconds each). Pure logic and Mockito-mocked repositories: `DuplicateEmailException`/`UserNotFoundException` thrown on the right conditions; `GlobalExceptionHandler` methods called directly with a constructed exception, asserting the `ProblemDetail`. Nothing here touches a database or a broker.
